...
@Inject constructor(
    ...
    override suspend fun getParticipantSources(
        ...
        val restSources = try {
            ...
            logger.warn(
                "Failed to fetch REST sources for project {}, participant {}, " + "continuing with RADAR sources only: {}",
                projectId,
                participantId,
                e,
            )
        ...
        val radarSources = try {
            radarSourcesClient.getParticipantSources(projectId, participantId)
        } catch (e: Exception) {
            logger.warn(
                "Failed to fetch RADAR sources for project {}, participant {}, " + "continuing with REST sources only: {}",
                projectId,
                participantId,
                e,
            )
        ...
        val combinedRest = restSources.map { rest ->
        ...
        val combinedRadar = radarSources.map { radar ->
            CombinedParticipantSource(
                projectId = projectId,
